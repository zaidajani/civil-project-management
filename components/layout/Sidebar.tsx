...
const navigation = [
  ...
  { href: "/pm/schedule", label: "Schedule", icon: "schedule" },
  { href: "/pm/ingestion", label: "Data Ingestion", icon: "ingestion" },
  ...
  { href: "/pm/communication", label: "Communication", icon: "communication" },
];
  ...
  return (
    <aside className="sidebar fixed top-0 left-0 z-40 h-screen w-64 flex flex-col border-r">
      <div className="flex h-16 items-center px-5 border-b">
        <Link href="/pm" className="flex items-center gap-2.5" aria-label="CivilManager Home">
      ...
      <nav className="flex-1 overflow-y-auto scrollbar-thin px-3 py-4 space-y-0.5" aria-label="Main navigation">
        {navigation.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`nav-item flex items-center gap-3 px-3 py-2.5 rounded-md focus-ring ${
                isActive ? "active" : ""
              }`}
