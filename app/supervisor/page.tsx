...
const statusColor: Record<string, string> = { Unassigned: "bg-status-atrisk-bg text-status-atrisk", Assigned: "bg-active-bg text-primary", "In Progress": "bg-active-bg text-primary", Completed: "bg-status-ontrack-bg text-status-ontrack", Blocked: "bg-status-delayed-bg text-status-delayed" };
...
export default function SupervisorPage() {
  ...
  const summary = useMemo(() => ({ unassigned: tasks.filter(t => t.status === "Unassigned").length, today: tasks.filter(t => t.dueDate === "2026-09-04").length, inProgress: tasks.filter(t => t.status === "In Progress" || t.status === "Assigned").length, availableLabourers: labourers.filter(labourer => labourer.availability === "Available").length }), [tasks]);
